...
{
	if (HINTERNET internet = InternetOpenA("copyrighttxt/BFU", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, NULL))
	{
...
{
	std::string response;
	if (!HttpRequest("https://api.github.com/repos/" BFU_GITHUB_REPO "/releases/latest", response))
	{
	...
	if (latest_version != BFU_VERSION)
	{
		char buffer[256];
		sprintf_s(buffer, "A new version of Bubbaverse FPS Unlocker is available.\n\nCurrent Version: %s\nLatest Version: %s\n\nVisit download page?", BFU_VERSION, latest_version.c_str());

		if (MessageBoxA(NULL, buffer, "Update Check", MB_YESNOCANCEL | MB_ICONEXCLAMATION) == IDYES)
		{
			ShellExecuteA(NULL, "open", "https://github.com/" BFU_GITHUB_REPO "/releases", NULL, NULL, SW_SHOWNORMAL);
			return true;
		}
	}
